...
- 只在明确授权后定位并重置指定 profile，保留启动器、当前指定节点及其他资料。换指纹不保证账号无法关联。

## 当前唯一出口要求

浏览器与 CLI 在任一时刻都应使用同一个当前 `expected_node`，其他叶子一律拒绝，不维护多节点允许列表。启用固定主备时，只有受控切换成功后，CLI 的当前目标才允许从主节点改为备用节点；浏览器是否支持同步迁移必须单独核对，不能仅因 CLI 已切换就假定浏览器也安全切换。链式代理只允许入口结构不同，不能改变当前最终出口。更新文档不等于已经修改或验证运行中代码。

## 历史运行代码核对（2026-09-05，尚未按当前唯一出口要求改造）
